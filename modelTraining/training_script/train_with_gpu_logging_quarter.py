...
if __name__ == "__main__":
    ...
    quarter_len = len(full_train) // 4
    train_data, _ = random_split(full_train, [quarter_len, len(full_train) - quarter_len])
    val_dataset = RatingDataset(val_dir)

    num_users = max(train_data.users.max().item(), val_dataset.users.max().item()) + 1
    num_items = max(train_data.items.max().item(), val_dataset.items.max().item()) + 1
